...
public class IterableTwoThreeTree280<K extends Comparable<? super K>, I extends Keyed280<K>> extends TwoThreeTree280<K, I> implements KeyedDict280<K,I> {
	...
	protected void delete(TwoThreeNode280<K, I> root, K keyToDelete ) {
		if( root.getLeftSubtree().isInternal() ) {
			...
			if( root.getLeftSubtree().getKey1().compareTo(keyToDelete) == 0 ) {
				...
				LinkedLeafTwoThreeNode280<K, I> leaf;
				leaf = (LinkedLeafTwoThreeNode280<K, I>) root.getLeftSubtree();
				
				// Link left and right of the deleting leaf if they are not null
				if(leaf.prev() != null && leaf.next() != null) {
					leaf.prev().setNext(leaf.next());
					leaf.next().setPrev(leaf.prev());
				} else if (leaf.prev() == null) {
					// If nothing to the left, left reference should be null
					leaf.next().setPrev(null);
				} else {
					// If nothing to the right, right reference should be null
					leaf.prev().setNext(null);
				}
				...
				LinkedLeafTwoThreeNode280<K, I> leaf;
				leaf = (LinkedLeafTwoThreeNode280<K, I>) root.getMiddleSubtree();
				...
				LinkedLeafTwoThreeNode280<K, I> leaf;
				leaf = (LinkedLeafTwoThreeNode280<K, I>) root.getRightSubtree();
	...
	/**
	 * Get the key of the data at the cursor
	 * @return Item key of type K
	 * @throws NoCurrentItem280Exception
	 */
	@Override
	public K itemKey() throws NoCurrentItem280Exception {
		
		if (cursor.getData().key() == null){
	...
	/**
	 * Get the item at which the cursor is positioned
	 * @return item of type I
	 * @throws NoCurrentItem280Exception
	 */
	@Override
	...
	/**
	 * Sets cursor to the position of a key if it's found, otherwise to the after position 
	 * @param k - item to be searched for
	 */
	@SuppressWarnings("unchecked")
	public void search(K k) {

		if(this.find(k)==null){
			// Set cursor to after if k isn't found
			this.cursor.setPrev(largest);
			this.cursor.setNext(null);
		} else {
			this.cursor = (LinkedLeafTwoThreeNode280<K, I>) this.find(k);
		}
	...
	/**
	 * Store an item at the cursor position
	 * @param x - item to be stored
	 * @throws NoCurrentItem280Exception
	 * @throws InvalidArgument280Exception
	 */
	@Override
	public void setItem(I x) throws NoCurrentItem280Exception,
			InvalidArgument280Exception {

		if (this.cursor.getData().key().compareTo(x.key()) != 0){
	...
	public void deleteItem() throws NoCurrentItem280Exception {
		...
		this.cursor.prev().setNext(cursor.next());
		this.cursor.next().setPrev(cursor.prev());
		this.cursor = this.cursor.next();
	}
